...
exports.DeleteOne=(req, res) =>{
    const id = req.params.id;
    club.destroy({where : {citeId: id}}).then((data) => {
        if (data == 1) {
...
exports.UpdateOne=(req, res) =>{
    const id = req.params.id;
    cite.findOne({where : {citeId: id}}).then((record) => {
        if (record) {
            record.update({
                nom: req.body.nom,
                type:req.body.type,
