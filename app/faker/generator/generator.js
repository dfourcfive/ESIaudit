...
exports.FakePfeMaster = () => {
  ...
  etudiant.findAll().then((data) => {
    for (let i = 0; i < data.length; i++) {
      ...
      var date_debut = faker.date.between(
        dates[i % 6] + "-01-01",
        dates[i % 6] + "-01-02"
      );
            ...
            .then((modules) => {
              for (let i = 0; i < etudiants.length; i++) {
                ...
                for (let j = 0; j < dmodules.length; j++) {
                  if (
                    etudiants[i].get("etudiantId") ==
                    dmodules[j].get("etudiantEtudiantId")
                  ) {
                    for (let m = 0; m < modules.length; m++) {
                      if (
                        dmodules[j].get("matiereMatiereId") ==
                        modules[m].get("matiereId")
                      ) {
                        if (m == 0) {
                    ...
                    totalMoyen =
                      totalMoyen +
                      dmodules[j].get("Moyenne") *
                        dmodules[j].get("Coefficient");
